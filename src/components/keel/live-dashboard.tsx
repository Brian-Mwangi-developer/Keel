...
import { getFeedLive, getLineageLive, getPipelineOverviewLive } from "@/lib/keel/client-fetch";
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-heading text-2xl font-semibold">Pipeline health</h1>
          <p className="flex items-center gap-1.5 text-sm text-muted-foreground">
            {overview.healthy_count} of {overview.asset_count} assets are safe to use. Live, computed from DataHub —
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatTile
          ...
          suffix="/ 100"
          tone={overview.band === "good" ? "good" : overview.band === "warn" ? "warn" : "bad"}
        ...
        <StatTile
          label="Movers"
          value={overview.movers.filter((m) => m.delta !== null && m.delta !== 0).length}
          suffix="changed"
          tone="default"
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-5">
        <Card className="xl:col-span-2">
          <CardHeader>
            ...
            <CardDescription>One score for the whole pipeline. Assets that more things depend on move it more.</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="xl:col-span-3">
          <CardHeader>
            ...
            <CardDescription>Click any node to see its full decomposition.</CardDescription>
          </CardHeader>
          <CardContent>
            {lineage && lineage.nodes.length > 0 ? (
              <LineageGraph lineage={lineage} />
            ) : (
              <p className="py-8 text-center text-sm text-muted-foreground">No lineage graph for this root yet.</p>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Sorted by change, not by current state.</CardDescription>
          </CardHeader>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {overview.movers.slice(0, 8).map((m) => (
                  ...
                  >
                    <td className="py-2">
                      <span className="font-medium hover:underline">{shortUrn(m.urn)}</span>
                    </td>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>MetadataChangeLog — real score and governance events, as they happen.</CardDescription>
          </CardHeader>
...
function DeltaBadge({ delta }: { delta: number | null }) {
  ...
  if (delta === 0) return <Minus className="ml-auto size-3.5 text-muted-foreground" />;
  const up = delta > 0;
  return (
    <span className={`inline-flex items-center gap-0.5 font-mono text-xs tabular-nums ${up ? "text-success" : "text-destructive"}`}>
